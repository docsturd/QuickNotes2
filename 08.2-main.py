...
from Subjective.subjective import *
from Objective.objective import *
from Objective.ObjectiveFunctions import *
# git change made this line more changes.  git change made this line more changes git change made this line more changes. git change made this line more changes. 


###  IMPORTANT FUNCTIONS LOAD AND WRITE  ###
#def load_json_data("patients_names.json"):
#    existing_data = []
#    try:
...
class CreatePatientPopup(QDialog):
    ...
    # Modify the stretch factors to set the width of the columns
        ...
        self.data = load_json_data("patients_names.json")
...
# Get screen size
        screen = QDesktopWidget().screenGeometry()
        width = int(screen.width() * .99)  # % of screen width
        height = int(screen.height() * 0.80)  # % of screen height
        self.resize(width, height)
        screen_pos = screen.topLeft() # Get the position of the screen's top-left corner
        self.move(screen_pos.x() + 5, screen_pos.y() + 110) # Move the window 100px down from the top of the screen
...
# Calculate the widths for the tab widget and text edit field
    ...
    def create_tabs(self):
        # Create 4 tabs
        tab_names = ['Subjective', 'Objective', 'Assessment', 'Plan']
        for i, name in enumerate(tab_names, start=1):
            if i == 1:
                tab = Subjective()
                tab.text_to_append.connect(self.append_text)
            elif i == 2:
                tab = Objective()
                tab.text_to_append.connect(self.append_text)
            elif i == 3:
                tab = SoapTabThreeAssessment()
            elif i == 4:
                tab = SoapTabFourPlan()
...
# class SoapTabTwoObjective(QWidget):
#     def __init__(self, parent=None):
#         super(SoapTabTwoObjective, self).__init__(parent)
...
#         self.setLayout(layout)
class SoapTabThreeAssessment(QWidget):
    def __init__(self, parent=None):
        super(SoapTabThreeAssessment, self).__init__(parent)
        layout = QVBoxLayout(self)
        ...
        layout.addWidget(self.widget2)
        self.setLayout(layout)
class SoapTabFourPlan(QWidget):
    def __init__(self, parent=None):
        super(SoapTabFourPlan, self).__init__(parent)
        layout = QVBoxLayout(self)
...
#####  Popup for exam
class ExampleTabsWithQtextedit(QDialog):

    def __init__(self, parent=None, patient=None):
        super(ExampleTabsWithQtextedit, self).__init__(parent)
# Get screen size
        screen = QDesktopWidget().screenGeometry()
        width = int(screen.width() * .95)  # 100% of screen width
        height = int(screen.height() * 0.75)  # 50% of screen height
...
class MenuBar(QMenuBar):
    ...
    def open_create_patient_popup(self):
        if self.main_window.create_patient_popup is None or not self.main_window.create_patient_popup.isVisible():
            self.main_window.create_patient_popup = CreatePatientPopup(self)
            self.main_window.create_patient_popup.show()
        else:
            self.main_window.create_patient_popup.raise_()
            self.main_window.create_patient_popup.activateWindow()
...
class MainWindow(QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.create_patient_popup = None  # Add this line
        ...
        self.document_text_edit = QTextEdit(self)
                # Assuming you have an instance of the Subjective class
        self.subjective_instance = Subjective()
        self.objective_instance = Objective()
        # Connect the custom signal to the slot
        self.subjective_instance.text_to_append.connect(self.update_text_edit)
        self.objective_instance.text_to_append.connect(self.update_text_edit)
...
# MENU BAR
    ...
    def update_text_edit(self, html_content):
        self.your_text_edit_widget.setHtml(html_content)
    def showEvent(self, event: QEvent):
        super().showEvent(event)
        if event.type() == QEvent.Show:
            self.create_patient_popup = CreatePatientPopup(self)
            self.create_patient_popup.show()
    def load_settings(self):
        try:
