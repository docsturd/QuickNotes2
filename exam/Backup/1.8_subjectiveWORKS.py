...
class Subjective(QWidget):
    ...
    def create_horizontal_line_with_spacer(self):
        """
    ...
    def create_group_box(self, title, symptoms):
        ...
        # complaint_layout, symptoms_dropdown, pain_severity_button_group = self.create_complaint_section(title, symptoms)
        ...
        vbox.addLayout(complaint_layout)
        vbox.addLayout(self.create_horizontal_line_with_spacer())
    ...
    def create_cheif_complaint_vas_section(self, title, symptoms):
        ...
        symptoms_dropdown.setObjectName(title)
        ...
        # return hbox
        return hbox, symptoms_dropdown, pain_severity_button_group
...
#
#
#         main_hbox = QHBoxLayout()  # This will contain all group boxes side by side
#
# # QUALITY
#         quality_groupbox = QGroupBox("Quality")
...
#             if item_counter >= MAX_ITEMS_PER_ROW:
#                 quality_vbox.addLayout(hbox_qualities)  # <- This line is changed
#                 hbox_qualities = QHBoxLayout()
#                 item_counter = 0
#
...
#             quality_vbox.addLayout(hbox_qualities)  # <- This line is changed too
#
#         quality_groupbox.setLayout(quality_vbox)
#         main_hbox.addWidget(quality_groupbox)  #
#
# # PROVOITIVE
#         provocative_groupbox = QGroupBox("Provocative")
#         provocative_vbox = QVBoxLayout()
#
#         provocative_list = ["Bending", "Standing", "Walking", "Lying down", "Twisting", "Reaching", "Pushing", "Pulling", "Lifting"]
#         provocative_list = [provocative.lower() for provocative in provocative_list]
#         provocative_list.sort()
#
#         MAX_ITEMS_PER_ROW = 2
#         item_counter = 0
#         hbox_provocative = QHBoxLayout()
#
#         for provocative in provocative_list:
#             if item_counter >= MAX_ITEMS_PER_ROW:
#                 provocative_vbox.addLayout(hbox_provocative)  # <- Add to provocative_vbox
#                 hbox_provocative = QHBoxLayout()
#                 item_counter = 0
#
#             checkbox = QCheckBox(provocative)
#             checkbox.stateChanged.connect(lambda state, checkbox=checkbox: self.set_provocative(checkbox.text(), state))
#             hbox_provocative.addWidget(checkbox)
#             item_counter += 1
#
#         if hbox_provocative.count() > 0:  # add remaining checkboxes if any
#             provocative_vbox.addLayout(hbox_provocative)  # <- Add to provocative_vbox
#
#         provocative_groupbox.setLayout(provocative_vbox)
#         main_hbox.addWidget(provocative_groupbox)  # Add the provocative group box to the main horizontal layout
#
#
# # INTERFERS
...
# # COMPLICATIONS
#         complications_groupbox = QGroupBox("Complications")
#         complications_vbox = QVBoxLayout()
#
#         complications_list = ["severe pain", "diabetes", "alcohol", "autoimmune", "age", "degenerative changes", "compliance", "work", "sedentary", "unrealistic expectations"]  # Replace with your actual complications
#         complications_list = [complication.lower() for complication in complications_list]
...
#             if item_counter >= MAX_ITEMS_PER_ROW:
#                 complications_vbox.addLayout(hbox_complications)  # <- Add to complications_vbox
#                 hbox_complications = QHBoxLayout()
#                 item_counter = 0
#
...
#
#         complications_groupbox.setLayout(complications_vbox)
#         main_hbox.addWidget(complications_groupbox)  # Add the complications group box to the main horizontal layout
#
#         vbox.addLayout(main_hbox)
# # BUTTONS
#         hbox_buttons = QHBoxLayout()
...
# SEVERITY
        if 1 <= pain_severity <= 10:
            severity_lambda = lambda severity: f"PainSeverity on a [1-10] scale is: {severity}/10."
...
# RADICULAR
        ...
        if radicular_pain_text:
            radicular_pain_text = f"RADICULAR: {radicular_pain_text}."
        else:
            radicular_pain_text = ""
...
# WHEN
        ...
        if when_it_happened_period in ['today', 'yesterday']:
            ...
            if not when_it_happened_number and when_it_happened_period in ["days", "weeks", "months", "years"]:
                ...
                when_it_happened_text = f"\nWHEN: {when_it_happened_number} {when_it_happened_period} ago."
            else:
                when_it_happened_text = ""

#MISSING TEXT
        # Emit full text
        full_text = complaint_text + severity_text + '\n' + complication_text + '\n' + quality_text + '\n' + provocative_text + '\n' + interfer_text + '\n' + severity_text + how_it_happened_text + when_it_happened_text + '\n' + radicular_pain_text + '\n' + Time_percentOfDay + '\n' + pain_status_text
